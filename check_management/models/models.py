...
class NewModule(models.Model):
    ...
    einv_is_submitted = fields.Boolean()


class PaymentCheck(models.Model):
    _name = "payment.check"
    _description = "PaymentCheck"
...
class PaymentCheckLine(models.Model):
    ...
    check_type = fields.Selection(
        [('outbound', 'Send'), ('inbound', 'Received'), ('transfer', 'Internal Transfer')], copy=False,
        string="Check Type", compute="_compute_check_type", store=True)
    ...
    check_accept_journal_id = fields.Many2one('account.journal', string="Collect Journal")
    deduct_journal_id = fields.Many2one('account.journal', string="Deduct Journal")
    ...
    text_amount = fields.Char(compute="amount_to_words")
    debit_journal_send_id = fields.Many2one("account.journal", string="Debit Journal", required=True)


    def open_partial_collection(self):
        return {
            'name': 'Partial Collection',
            'view_type': 'form',
            'view_mode': 'form',
            'target': 'new',
            'res_model': 'partial.collection.wizard',
            'type': 'ir.actions.act_window',
            'context': {'default_wizard_partial_amount': self.check_amount, 'default_payment_check_line_id': self.id, },
        }

    def return_partial_collection(self):
        for rec in self:
            return {
                'name': 'Partial Collection',
                'view_type': 'form',
                ...
                'res_model': 'partial.collection.wizard',
                'type': 'ir.actions.act_window',
                'context': {'default_wizard_partial_amount': self.check_amount,'default_credit_journal_id': self.debit_journal_send_id.id,
                            'default_payment_check_line_id': self.id, },
            }


    @api.constrains('check_number', 'check_bank_id')
    def check_check_number_bank(self):
        for rec in self:
            if rec.check_number and rec.check_bank_id:
                if self.env['payment.check.line'].search(
                        [('check_number', '=', rec.check_number), ('check_bank_id', '=', rec.check_bank_id.id),
                         ('id', '!=', rec.id)]):
                    raise ValidationError(_('Check Number and Bank must be unique'))

    @api.depends('check_amount')
    def amount_to_words(self):
        for rec in self:
            rec.text_amount = rec.payment_id.currency_id.amount_to_text(rec.check_amount).replace('and',
                                                                                                  'و') + " فقط لا غير"
    ...
    def reset_holding(self):
        for rec in self:
            rec.state = 'holding'
            for line in rec.move_ids.filtered(lambda x: x.state != 'cancel'):
                line.button_cancel()
    ...
    def unlink(self):
        for rec in self:
            if rec.state != 'holding':
                raise UserError("Sorry, you can not delete this cheque it should be in holding state")
