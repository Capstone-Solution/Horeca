...
class AccountPayment(models.Model):
    ...
    def action_draft(self):
        ...
        if all(line.state in ['draft', 'cancel'] for line in self.payment_check_lines.move_ids):
            self.payment_check_lines.reset_holding()
        else:
            raise UserError("You must cancel checks transactions")
        return res

    def unlink(self):
        if any(line.state != 'holding' for line in self.payment_check_lines):
            raise UserError("You must cancel checks transactions")
        else:
            self.payment_check_lines.unlink()
        return super().unlink()
